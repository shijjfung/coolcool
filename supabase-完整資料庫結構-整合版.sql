...
CREATE TABLE IF NOT EXISTS forms (
  ...
  line_comment_url TEXT,
  -- Facebook 自動監控設定
  facebook_post_url TEXT,
  facebook_post_author TEXT,
  facebook_keywords TEXT,
  facebook_auto_monitor INTEGER DEFAULT 0,
  facebook_reply_message TEXT,
  -- LINE 自動監控設定
  line_post_author TEXT
);
...
CREATE INDEX IF NOT EXISTS idx_forms_facebook_auto_monitor ON forms(facebook_auto_monitor);
CREATE INDEX IF NOT EXISTS idx_forms_line_post_author ON forms(line_post_author);
...
-- line_posts 表索引（只有在表存在且有 created_at 欄位時才建立）
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'line_posts') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns
               WHERE table_name = 'line_posts' AND column_name = 'created_at') THEN
...
-- 確保 line_posts 表有 created_at 欄位（如果表已存在但缺少欄位）
DO $$
BEGIN
  -- 如果 line_posts 表存在但沒有 created_at 欄位，添加它
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'line_posts') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name = 'line_posts' AND column_name = 'created_at') THEN
      ALTER TABLE line_posts ADD COLUMN created_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
  END IF;
END $$;

-- 確保所有表都有必要的欄位後，再建立索引
-- 如果索引已存在，這些語句不會出錯（使用 IF NOT EXISTS）
