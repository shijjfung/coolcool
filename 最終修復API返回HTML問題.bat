...
echo [步驟 2] 提交修復...
git commit -m "最終修復：確保所有 API 返回 JSON

- 在每個返回語句前都設置 JSON 響應頭
- 使用獨立的 setJsonHeaders 函數
- 確保所有錯誤路徑都返回 JSON
- 改進錯誤訊息，提示檢查環境變數" 2>&1
echo.
...
echo 如果環境變數已設定，應該返回 JSON
echo 如果環境變數未設定，應該返回 JSON 錯誤訊息（不是 HTML）
echo.
echo ========================================
echo.
