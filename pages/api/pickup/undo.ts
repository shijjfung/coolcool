...
import { ensureDatabaseInitialized, undoPickupItem, PickupStatusFilter } from '@/lib/db';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { token, orderId, itemKey, status } = req.body || {};
  ...
  try {
    ...
    const { orders } = await undoPickupItem(token.trim(), orderIdNumber, itemKey.trim(), statusFilter);
    return res.status(200).json({ success: true, status: statusFilter, orders });
  } catch (error: any) {
    console.error('pickup undo error', error);
    return res.status(400).json({
      success: false,
      error: error?.message || '取消取貨紀錄失敗',
    });
  }
}
