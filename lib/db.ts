...
// ?????ËºØÊ???Supabase?Ó§?£Æ???Supabase ?Ó°øÓ??
let dbModule: any;
...
// SQLite ?Ó°øÓ???Ó§???Ë¨???????
// ??Ë≥ÉÁ? SQLite ??¨Ô???Ó≥?????????????Vercel (Supabase) ????Ó≥????let sqlite3: any;
let promisify: any;
let path: any;
let fs: any;
let db: any;
...
if (DATABASE_TYPE === 'sqlite') {
  // ????Ó∫???SQLite ????????Ó§?Ëπ??Ó§¢Á? Supabase ??¨Ô???Ó≥?????
  ...
  // ??????êÔî¢???Ω¬Ä??ÔΩ????  if (!fs.existsSync(dbPath)) {
    fs.writeFileSync(dbPath, '');
  }

  db = new sqlite3.Database(dbPath);

  // ??callback ?Ë©®Ôà§Ëµ????????Ë¨úÓê∫??Promise
...
// ?Ë±??Ë¨??Ë¨ïÓö¶???async function initDatabaseSQLite() {
  // ??ÑÔ?Ë¨òÔÜ≥Ó°????  await dbRun(`
  ...
  // ?ÊΩòÊ?Ë≤îÓºø??µÔ????ÆÁ?Ë¨òÔã´????Ó§???Á∂ΩÓ£∞Ë¨ÜÓ??∂Ê??
  try {
  ...
  // LINE ????ÊÆâÊ≠ª??????ÊÆâÊ≠ª???????Ç¬Ä????Ó∫???Ë¨???  await dbRun(`
...
export interface Form {
  ...
  fields: FormField[];
  deadline: string;
  order_deadline?: string; // ???????Áø?Ëπ?
  order_limit?: number; // ?ÊÆ?Ë¨????????Ó§?????
  pickup_time?: string; // ?Ë¨òËºª??Ëπ??Ó§?????
  report_generated?: number; // ??áËÉº???Áß???ΩÔ????(0/1)
  report_generated_at?: string; // ??áËÉº??Ë≥??Ëπ?
  deleted?: number; // ??Áß???ΩÔ????????????(0/1)
  deleted_at?: string; // ?????Ëπ?
  created_at: string;
  form_token: string;
  facebook_comment_url?: string;
  line_comment_url?: string;
  facebook_post_url?: string; // Facebook ??ïËî¨?????Ó§????èË?????????
  facebook_post_author?: string; // Facebook ????????  facebook_keywords?: string; // Facebook ?Ë¨öÊÆ∑Ó®?ÊÆ??????JSON ?????  facebook_auto_monitor?: number; // ??Áß??Ë≥πÔÅæ? Facebook ???????(0/1)
  facebook_reply_message?: string; // Facebook ?Ë±??ÊÆ?Ëπ?  line_post_author?: string; // LINE ???????????Ë∏???∏Êú±??êÓ?ËπìË??????????}
...
/**
 * ?Ë¨???ΩÔ????????ÊΩ???≥Ê??????Ó∫ßÂûÆË¨?Ë≥???áËÉº????ú¬Ä?? * ?ËºØÊ???order_deadline?Ó§???Ë¨?????Ó§?£Æ???????deadline
 */
async function getFormsReadyForReportSQLite(): Promise<Form[]> {
  const now = new Date().toISOString();
  // ????ËºØÊ???order_deadline?Ó§?£Æ??Ë¨?????ËºØÊ???deadline
  ...
  return rows.map(row => ({
    ...
    fields: JSON.parse(row.fields),
    deadline: row.deadline,
    order_deadline: row.order_deadline || row.deadline, // ?????Ó≥?? order_deadline?Ó§?£≠???deadline
    report_generated: row.report_generated || 0,
    report_generated_at: row.report_generated_at || undefined,
    created_at: row.created_at,
    form_token: row.form_token,
...
async function moveFormToTrashSQLite(formId: number): Promise<boolean> {
  try {
    // ??????êÔî¢???Ω¬ÄÓ∫??Ë±???    await ensureDatabaseInitialized();
    
    // ??????????Ë¨¢¬Ä??µ¬Ä???    const tableInfo = await dbAll("PRAGMA table_info(forms)") as any[];
    ...
    // ?ËºØÊ???ISO ????????‰Ω???    const deletedAt = new Date().toISOString();
    
    // ??????Ë¨?????????    let result: any = null;
    try {
      result = await dbRun(
      ...
      if (error.message && error.message.includes('no such column')) {
        try {
          ...
          throw new Error(`?????ùÓ????ÑÔ?Ë¨òÔÜØ??{e2.message}`);
        }
      } else {
        throw error;
      }
    ...
    // ???? changes ??0?Ó§?£≤???πÓ?Ë¨¢¬Ä????????????    if (changes === 0) {
      const checkForm = await dbGet('SELECT deleted FROM forms WHERE id = ?', [formId]) as any;
      if (checkForm && checkForm.deleted === 1) {
        return true;
      }
    ...
    return changes > 0;
        ...
        console.error('Failed to load Supabase module');
        console.error('Failed to load Supabase module');
    throw new Error(`?????ùÓ????ÑÔ?Ë¨òÔÜØ??{error.message}`);
  }
}
...
async function restoreFormSQLite(formId: number): Promise<boolean> {
  try {
    ...
    return changes > 0;
        ...
        console.error('Failed to load Supabase module');
    throw new Error(`????????ÑÔ?Ë¨òÔÜØ??{error.message}`);
  }
}

/**
 * ?ÓµßÈò≤???????ÑÔ?Ë¨òÔÜØ????ËπáÓ????Ê≠??Ë¨????
 */
async function permanentlyDeleteFormSQLite(formId: number): Promise<{ success: boolean; deletedOrders: number }> {
  // ????Óµ???????????ÊæóÓ¥±??  const orderCount = await dbGet('SELECT COUNT(*) as count FROM orders WHERE form_id = ?', [formId]) as { count: number };
  const deletedOrders = orderCount?.count || 0;

  // ????????ÊÆ?Ë¨?  await dbRun('DELETE FROM orders WHERE form_id = ?', [formId]);

  // ?ÓµßÈò≤???????ÑÔ?Ë¨?  const result = await dbRun('DELETE FROM forms WHERE id = ?', [formId]);
...
export interface Order {
  ...
  items_summary?: Array<{ name: string; quantity: number }>; // ????Ó∞¥Ô?Ë¨òÔÜØ????Ó∏ÑÈô™???üÂû¢?§Âà∏????????Ë¨?
  client_ip?: string;
  user_agent?: string;
  created_at: string;
  updated_at: string;
...
async function createOrderSQLite(
  ...
  userAgent?: string,
  orderSource?: string,
  form?: Form // ??? form ??Â°óÓ¥±??Ë∏????????Ó∞¥Ô?Ë¨?): Promise<string> {
  const orderToken = generateToken();
  
  // ???????Ó∞¥Ô?Ë¨?  let itemsSummary: string | null = null;
  if (form) {
  ...
  const result = await dbRun(
    ...
    if (!result) {
      ...
      if (checkOrder && checkOrder.order_token === orderToken) {
        // ?Ë∏????êÔî¢???Áß??Ë≥???ùÓ????        const verifyOrder = await dbGet('SELECT order_data FROM orders WHERE order_token = ?', [orderToken]) as any;
        if (verifyOrder) {
          const currentData = JSON.parse(verifyOrder.order_data);
          // ??¨Ô?Ë¨òÔÜ±Ó•???Ó§????ÜÓ£∞??Ó§?£≠???????¥È¨≤Ëπ??ûË?Áß?Ë∏????          if (JSON.stringify(currentData) === JSON.stringify(orderData)) {
            return true;
          }
        }
      }
    ...
    return changes > 0;
        ...
        console.error('Failed to load Supabase module');
    throw new Error(`?????ùÓ???ÊÆ?Ë¨òÔÜØ??{error.message}`);
  }
}
...
async function deleteOrderSQLite(orderToken: string): Promise<boolean> {
  try {
    ...
    return changes > 0;
        ...
        console.error('Failed to load Supabase module');
    throw new Error(`????????ÊÆ?Ë¨òÔÜØ??{error.message}`);
  }
}
...
// ?Ë∏???ÊÆ?Ë¨????Ó§?QLite??async function reserveOrderNumberSQLite(formId: number, sessionId: string): Promise<{ success: boolean; orderNumber?: number; error?: string }> {
  try {
    await ensureDatabaseInitialized();
    
    // ???????Ë≥π¬Ä??Ë¨?5?????    const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000).toISOString();
    await dbRun(
    ...
    if (existing) {
      // ?ÊΩòÊ?Ë≤??Áß???ΩÔ????      const reservedAt = new Date(existing.reserved_at);
      const now = new Date();
      if (now.getTime() - reservedAt.getTime() > 5 * 60 * 1000 && !existing.order_token) {
        // ??ΩÔ???Ë≥??????Ó≥ãÓ•≤??????        await dbRun('DELETE FROM reserved_orders WHERE id = ?', [existing.id]);
      } else {
        // ?óÔ???????Ë∏??
    ...
    // ?Ë¨??????ΩÔ????úÓ†ª??ÊÆ?Ë¨??ÁÆ??Ë∏?????ÆÓ¥±??    const orders = await dbAll('SELECT id FROM orders WHERE form_id = ?', [formId]) as any[];
    ...
    // ?ÊÆ???Ó≥????Ó≥êÓ?????????    const usedNumbers = new Set<number>();
    reserved.forEach((r: any) => {
      usedNumbers.add(r.order_number);
    });

    // ?????Ó´????Ó≥êÓ?????????    let orderNumber = 1;
    while (usedNumbers.has(orderNumber)) {
      orderNumber++;
    }

    // ???êÔÖπ?Ë∏???ÊÆâÊ≠ª??Ó§????INSERT OR REPLACE ???????Ó≥????    await dbRun(
...
// LINE ????ÊÆâÊ≠ª???????Ó¥?Ó§?QLite ???≤Áßß??async function recordLinePostSQLite(
  formId: number,
  groupId: string,
  ...
  try {
    ...
    await dbRun(
      ...
      [formId, groupId, messageId, senderName, postContent]
        ...
        console.error('Failed to load Supabase module');
    throw new Error(`?ÊÆâÊ≠ª? LINE ?????????{error.message}`);
  }
}
...
// ?Ë±??Ë¨??Ë¨ïÓö¶??Ó§?Ë¨ìÂ??????ÉÔ???????πÊ?????let dbInitialized = false;
export async function ensureDatabaseInitialized() {
  if (DATABASE_TYPE === 'supabase') {
    // Supabase ?Ó≥???ÁßãÊ???Ëø???Ó§?£±?Ó≥äÓ?????ΩÔ?Ë¨?SQL Editor ?Ó≥???Ó≤?    // ?????ÁßãÊí≠??øÓ??dbModule ??ΩÔ????    if (!dbModule) {
      throw new Error('Supabase ???????????ÔΩ???øÓ™Ø?Ó©???ØÓ????∞ÂÜΩ?????øÓ???');
    }
...
// ?????êÔî¢?????????????Ó∫????????// ?????ËºØÊ???Supabase?Ó§?£Æ? dbModule ??Ó¶??Ó∫??????????// ?????ËºØÊ???SQLite?Ó§?£≠????????Ôº?? SQLite ???Ó¥?
...
// ?Ë∏???ÊÆ?Ë¨?????????Ó¥?export const reserveOrderNumber = DATABASE_TYPE === 'supabase'
...
// LINE ????ÊÆâÊ≠ª???????Ó¥?Ó§?QLite??async function recordLinePostSQLite(
  formId: number,
  groupId: string,
  messageId: string | null,
  senderName: string,
...
// LINE ????ÊÆâÊ≠ª???????Ó¥?export const recordLinePost = DATABASE_TYPE === 'supabase'
...
// generateSessionId ??ΩÔ?Ë¨ìÂ???????äÂ?????µ¬Ä???
